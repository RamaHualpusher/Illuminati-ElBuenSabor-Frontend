...
const Navbar: FC = () => {
  const { isAuthenticated, logout } = useAuth0(); // Obtener el estado de autenticación de Auth0
  ...
  useEffect(() => {
    ...
    if (usuarioContext && !usuarioContext.activo) {
      // Mostrar el toast de advertencia
      setShowInactiveToast(true);

      // Desloguear al usuario después de 5 segundos
  ...
  return (
    <>
      ...
      <Toast
        ...
        style={{
          position: "fixed",
          top: "90px", // Ajusta la posición vertical según tu diseño
          right: "10px", // Ajusta la posición horizontal según tu diseño
          zIndex: 1000, // Asegúrate de que esté por encima del navbar
        }}
      >
        ...
        <Toast.Body className="toast-body fs-5">
          El usuario está Bloqueado. Se cerrará la sesión en 3 segundos.
        </Toast.Body>
      </Toast>
    </>
  );
